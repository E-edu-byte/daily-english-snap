            ...
            <div className="flex items-center gap-2">
              ...
              <a
                href={(() => {
                  ...
                  const text = `完全無料、登録不要の英語学習サイト\n\n【今日の英語クイズ - ${LEVEL_CONFIG[level].label}】\n\n「${blankPhrase}」\n（${phrase.meaning}）\n\n空欄に入る単語は？\nわかったら♡を押してね\n正解はリンクから！\nディクテーションもあるよ！\n\n`
                  return `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(shareUrl)}&hashtags=英語学習,英会話,朝活,すきま時間`
                })()}
                target="_blank"
                    ...
                    if (personMatch) {
                      ...
                      return (
                        <div key={lineIndex} className={`relative transition-all ${
                          ...
                          {/* 行完了時のはなまる（音声ボタン付近に重ねて表示） */}
                          {lineComplete && (
                            <div className="absolute top-0 right-0 flex items-center gap-1 animate-bounce-in z-10">
                          ...
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <button
                              ...
                              className="ml-7 px-1.5 py-0.5 hover:bg-emerald-200 bg-emerald-100 rounded text-[10px] font-bold text-emerald-700 transition-all hover-scale flex-shrink-0"
